...
export function registerWebApi(ctx, { deps, store, graph, research: researchDeps, capabilities, defaultMode, log }) {
  ...
  try {
    // ctx.effect() owns the disposer lifecycle. kind "prefix" (as dsh-godmode
    // mounts /godmode/api) so /kestrel/api/<method> sub-paths reach the handler
    // — an "exact" route matches only /kestrel/api itself, so the browser's
    // /verify and /research POSTs were answered 405 by the app before us.
    ctx?.effect?.(
      () => webServer.register({ kind: 'prefix', path: KESTREL_API_PATH, handler: handleRoute }),
