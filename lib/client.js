    ...
    function VeritasMessageActionButton(props) {
      ...
      const runVerification = useCallback(async () => {
        ...
        try {
          ...
          // Strategy 1: Extract from rendered DOM relative to button
          if (btn) {
            try {
              const turnTail = btn.closest("[data-turn-tail]");
              const turnContainer = turnTail ? turnTail.parentElement : btn.closest(".chat-turn") || btn.closest("article") || btn.parentElement?.parentElement?.parentElement;
              if (turnContainer) {
                const links = turnContainer.querySelectorAll("a[href^='http']");
                for (const a of links) {
                  if (a.href) foundUrls.add(a.href);
                }
                const clone = turnContainer.cloneNode(true);
                const tail = clone.querySelector("[data-turn-tail]");
                if (tail) tail.remove();
                textToVerify = (clone.innerText || clone.textContent || "").trim();
              }
            } catch {
              /* ignore */
            }
          ...
          // Strategy 2: Extract from props.useChat selector hook
          if (!textToVerify || textToVerify.length < 20) {
            try {
              const chatSnapshot = props.useChat ? props.useChat((s) => s) : null;
              if (chatSnapshot?.nodes?.values) {
                const nodes = chatSnapshot.nodes.values();
                const matching = nodes.find((n) => n && (n.messageId === messageId || (n.kind === "assistant" && n.messageId === messageId)));
                const targetNode = matching || nodes.filter((n) => n && n.kind === "assistant").at(-1);
                if (targetNode?.blocks) {
                  textToVerify = targetNode.blocks
          ...
          // Strategy 3: Global DOM fallback for latest rendered prose
          if (!textToVerify || textToVerify.length < 20) {
            try {
              const allParas = Array.from(document.querySelectorAll("article, [data-turn], p, li"))
                .map((el) => el.textContent?.trim() || "")
                .filter((t) => t.length > 30);
              if (allParas.length > 0) {
                textToVerify = allParas.slice(-20).join("\n");
              }
            } catch {
              /* ignore */
            }
          ...
          if (!textToVerify || textToVerify.length < 15) {
            setReport({
              totalSources: 0,
              independentOrigins: 0,
              syndicationsCollapsed: 0,
              phantomCount: 0,
              anchoredPct: 100,
              tribunalStatus: "Unverified",
              summary: "No message text found in container to audit.",
              claims: [
                { id: "c1", text: "Unable to locate message text.", verdict: "UNVERIFIED", ics: 0, anchored: false }
              ],
            });
            setShowDetail(true);
            return;
          ...
          // Try Host RPC if running inside Cordis harness
          let hostData = null;
          try {
          ...
          // Compute real domain uniqueness & Independent Corroboration Score
          ...
          // Extract substantive claims from the actual text
          const rawLines = textToVerify
            .split(/\n+/)
            .map((l) => l.replace(/^[#*|\s>-]+/, "").trim())
            .filter((l) => l.length > 25 && !l.startsWith("http") && !l.startsWith("---") && !l.startsWith("```"));

          const substantive = rawLines.filter((l) =>
            /\b(egg|protein|leucine|muscle|synthesis|intake|g\/kg|study|found|increase|percent|%|yield|demonstrat|fish|beef|salmon|whey|amino|mTOR|diaas|metabol|acquired|consideration|billion|million)\b/i.test(l)
          );

          const candidateLines = substantive.length > 0 ? substantive.slice(0, 4) : rawLines.slice(0, 3);
          const finalClaims = candidateLines.map((line, idx) => {
            const short = line.length > 85 ? line.slice(0, 82) + "..." : line;
            const isContradicted = /\b(refuted|contradicted|retracted|false|untrue|overturned|restructured)\b/i.test(line);
            return {
              id: `c${idx + 1}`,
              text: short,
