    ...
    function KestrelComposerButton(props) {
      ...
      const toggleMode = useCallback(() => {
        ...
        try {
          ...
          if (next) {
            if (!currentText.startsWith("/deep_research") && !currentText.includes("[Kestrel]")) {
              updateDraft(`[Kestrel Deep Research] ${currentText}`);
            }
          } else {
            if (currentText.startsWith("[Kestrel Deep Research] ")) {
              updateDraft(currentText.replace("[Kestrel Deep Research] ", ""));
            }
          }
        } catch {
      ...
      return h("button", {
        type: "button",
        className: `kestrel-composer-btn ${isActive ? "active" : ""}`,
        onClick: toggleMode,
        title: isActive
          ? "Deep Research Mode active: Routes prompts through adversarial tribunal (click to turn off)"
          : "Toggle Deep Research Mode (Kestrel)",
      },
        h("span", { className: "kestrel-composer-indicator" }),
        h(CompassIcon),
      ...
      const runVerification = useCallback(async () => {
        ...
        try {
          ...
          if (typeof host === "undefined" || typeof host.call !== "function") {
            setError("The Kestrel host engine is not reachable from this page.");
            setShowDetail(true);
            return;
          }
