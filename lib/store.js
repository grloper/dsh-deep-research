/**
 * @file Kestrel — Persistent evidence store (mechanism M5, the compounding moat).
 * @license MIT
 *
 * Perplexity restarts from zero on every query, forever. This store is why
 * Kestrel does not: verified claims, their evidence, their lineage and their
...
export function defaultStorePath(homeDir = process.env.DSH_HOME || process.env.HOME || process.env.USERPROFILE || '.') {
  const current = join(homeDir, '.dsh', 'kestrel', 'evidence.db')
  migrateLegacyStore(homeDir, current)
  return current
}

/**
 * Move a pre-rename evidence store to its current location, once.
 *
 * The project was called VERITAS before v0.3 and stored evidence under
 * `.dsh/veritas/`. The whole point of the store is that findings accumulate
 * across runs, so silently starting from an empty database after an upgrade
 * would discard exactly the thing users were promised would persist.
 *
 * Deliberately conservative: it only acts when the legacy file exists and the
 * current one does not, it never deletes the original, and any failure is
 * swallowed because a migration must never prevent the plugin from loading.
 *
 * @param {string} homeDir
 * @param {string} currentPath
 * @returns {boolean} whether a legacy store was copied forward
 */
export function migrateLegacyStore(homeDir, currentPath) {
  try {
    const legacy = join(homeDir, '.dsh', 'veritas', 'evidence.db')
    if (!existsSync(legacy) || existsSync(currentPath)) return false
    mkdirSync(dirname(currentPath), { recursive: true })
    copyFileSync(legacy, currentPath)
    // Carry the SQLite sidecars across too, or a WAL-mode database can be
    // reopened without its most recent committed transactions.
    for (const suffix of ['-wal', '-shm']) {
      if (existsSync(legacy + suffix)) copyFileSync(legacy + suffix, currentPath + suffix)
    }
    return true
  } catch {
    // A failed migration is not fatal: the plugin starts with an empty store.
    return false
  }
}
