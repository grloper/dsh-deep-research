/**
 * @file VERITAS — Mechanical citation anchoring (mechanism M1).
 * @license MIT
 *
 ...
 * hallucinated. VERITAS instead requires each claim to carry a *verbatim quote*
...
export function anchorQuote(quote, docText, opts = {}) {
  ...
  if (allowFuzzy) {
    ...
    if (qTokens.length > 0 && dTokens.length >= qTokens.length) {
      ...
      for (let i = 0; i + win <= dTokens.length; i++) {
        const j = tokenJaccard(qTokens, dTokens.slice(i, i + win))
        if (j > best) {
          best = j
          bestStart = i
        }
