/**
 * @file Kestrel — Claim verification pipeline (`verify_text`).
 * @license MIT
 *
...
export async function verifyText(text, deps = {}) {
  ...
  for (const claim of claims) {
    ...
    for (const doc of docs) {
      ...
      // A NEUTRAL document mentions the topic without bearing on the claim. It
      // must never appear in the citation list: rendering it would attach a
      // source to a claim it does not support, which is precisely the
      // "citation ≠ support" failure this engine exists to eliminate. Only
      // documents that actually adjudicate the claim become citations.
      const isProbative =
      ...
      if (!isProbative) {
        notes.push(`${doc.url ?? doc.id} mentions the topic but does not address the claim.`)
        continue
      }
...
export function renderReport(report) {
  ...
  for (const j of report.claims) {
    ...
    for (const c of j.citations) {
      // Stance is explicit: a reader must never mistake a refuting source for a
      // supporting one just because both appear under the same claim.
      const stance = c.stance && c.stance !== Verdict.SUPPORTED ? ` — ${c.stance}` : ''
      lines.push(`- [${c.anchorKind}${stance}] ${c.url ?? c.docId} (credibility ${c.credibility}/100)`)
      lines.push(`  > "${c.quote}"`)
    }
